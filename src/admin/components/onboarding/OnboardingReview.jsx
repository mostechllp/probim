...
const OnboardingReview = () => {
  ...
  // ─── ONLY COMPLETE ONBOARDING - NO SALARY/BANK SAVES ──────────────────
  ...
  // ─── Get the employee's user ID from localStorage or state ─────────────
  const getUserId = () => {
  ...
  // ─── Handle Submit - ONLY COMPLETE ONBOARDING ─────────────────────────
  const handleSubmit = async () => {
    ...
    try {
      ...
      // This will mark the employee as 'active' instead of 'onboarding'
      try {
        ...
        // Navigate to employees list or dashboard after success
  ...
  const handleBack = () => {
    dispatch(setStep(4)); // Go back to Step 4 (Salary & Bank Details)
  };
  ...
  const getBankAccounts = () => {
    ...
    if (salaryBankDetails?.bankAccounts && salaryBankDetails.bankAccounts.length > 0) {
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider flex items-center gap-2">
                    <FiPackage size={14} /> Salary Packages
                  </p>
